...
- **Feb 10, 2026**: **Mandala watermark added to Home Dashboard** — user-provided starry-night mandala image saved to `/app/frontend/src/assets/mandala-watermark.png` and rendered as a fixed, subtle (~8% opacity) watermark on the right side of `HomePage.jsx`. New `.mandala-watermark` class in `shared-pages.css` with radial fade mask, gentle drift animation, dark-mode boost, and mobile responsive scaling. Zero impact on content readability — pure "customized signature" touch.
