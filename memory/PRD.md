...
- **Feb 10, 2026**: **Games page — 2-column layout redesign + neon-gradient icons** — restructured `ConcentrationGamesPage.jsx` from vertical-stack + narrow sidebar to a 50/50 grid: LEFT column features a `focus-visual` hero card (new geometric mandala image at `/app/frontend/src/assets/focus-mandala.png` with baked-in "25:00 Deep Work Sprint" timer) followed by AI Insight banner + Level bar; RIGHT column has "TODAY'S CHALLENGES" heading + 5 challenge cards + AI Coach card below. Replaced emoji icons in CHALLENGES data with lucide-react components (Brain, Zap, Waves, Target, Dumbbell) rendered in new `.ch-icon-neon` boxes with per-challenge linear-gradient backgrounds + colored drop-shadow glows.
