...
- **Feb 10, 2026**: **Games page — 2-column layout redesign + neon-gradient icons** — restructured `ConcentrationGamesPage.jsx` from vertical-stack + narrow sidebar to a 50/50 grid: LEFT column features a `focus-visual` hero card (mandala backdrop + big `25:00` timer + "Deep Work Sprint" label) followed by AI Insight banner + Level bar; RIGHT column has "TODAY'S CHALLENGES" heading + 5 challenge cards + AI Coach card below. Replaced emoji icons in CHALLENGES data with lucide-react components (Brain, Zap, Waves, Target, Dumbbell) rendered in new `.ch-icon-neon` boxes with per-challenge linear-gradient backgrounds + colored drop-shadow glows (pink→magenta for Memory, lime→green for Speed, cyan→blue for Deep Work, orange→red for Single Tab, amber→gold for Micro Workout). Added responsive stacking for mobile (`< 900px`) and `prefers-reduced-motion` respect on the mandala rotation.
