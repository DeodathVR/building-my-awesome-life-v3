...
- **Feb 10, 2026**: **Landing hero text contrast fix** — reduced Mandala watermark opacity from 35% → 18% on landing page, plus added a radial vignette (`radial-gradient(ellipse at 50% 32%, var(--bg) 0%, transparent 55%)`) fixed behind the hero copy zone so text stays crisp over the mandala. Also extended `.dark .alu-s` scoped dark-mode overrides to include `[data-testid="landing-page"]` so the public landing page renders correctly in both light and dark themes. Verified via Playwright: hero text is now fully legible against the mandala backdrop.
