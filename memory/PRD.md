...
- **Feb 10, 2026**: **Home Page dark mode fix** — added scoped `.dark .alu-s[data-testid="home-page"]` CSS variable overrides in `shared-pages.css` so dark mode now covers the full HomePage (background, cards, text, borders, shadows) instead of only the top navigation. Palette mirrors the app's `.dark` tokens from `index.css`. Verified 100% by testing agent (iteration_3).
- **Feb 10, 2026**: **Public/Protected hybrid access (Phases 1-3)** — big refactor to open the app up for logged-out visitors:
  - New `LandingPage.jsx` at `/` (Mandala hero + 8 feature grid + testimonials + CTA); logged-in users redirect to `/dashboard` (the existing HomePage)
  - Awesome Feed (`/feed`) — public read-only; like/save/share/bloom → signup toast prompt
  - Community (`/community`) — public browse; post/like/reply → signup toast prompt
  - Success Conspiracy (`/conspiracy`) — 2 free AI reframes/thought-flips per anonymous visitor (tracked in `localStorage.alu_anon_conspiracy_uses`), then signup wall
  - Concentration Games (`/concentration-games`) — 5 free games (Number Flash · Math Sprint · Box Breathing · One Focus · Ten Rep) playable anonymously; other 10 games redirect to /pricing. AI Coach returns "Sign up to chat" for anonymous users
  - Glow Up (`/glow-up`) — replaced upload flow with a **curated demo gallery** (4 wellness goals × before/after Unsplash images) so anonymous visitors see the vision at zero AI cost
  - `Navigation.jsx` — dynamic nav: Home→/dashboard when logged in, /→landing when logged out; hides Habits/Focus/AICoach nav items for anonymous users
  - `RouteGuards.jsx` — Admin/PublicOnly redirects go to `/dashboard`
- **Feb 10, 2026**: **Security hardening** — new `/app/frontend/vercel.json` adds Content-Security-Policy, X-Frame-Options: DENY, HSTS (2 years + preload), X-Content-Type-Options: nosniff, Referrer-Policy: strict-origin-when-cross-origin, Permissions-Policy (camera=(), microphone=(self), geo=(), payment=(self)). New `/app/frontend/src/lib/sanitize.js` strips `<script>/<style>` inner content + all HTML tags + dangerous URL schemes (javascript:/data:/vbscript:/file:) from user-generated Community posts and Success Conspiracy AI prompts. `firestore.rules` updated to allow public read on `feed_posts`, `community_posts`, `challenges` (needs manual Firebase Console redeploy). `AwesomeFeedPage` has an in-code FALLBACK_FEED (6 evergreen posts) so anonymous visitors always see content even before rule propagation. Verified 100% by testing agent (iteration_5).
- **Feb 10, 2026**: **SEO layer (Google Search Console ready)** — new lightweight `useSEO` hook in `/app/frontend/src/hooks/useSEO.js` updates `document.title`, meta description, canonical URL, OpenGraph (og:title/description/url/image + dimensions), and Twitter Card tags on every route change. Applied to all 11 public routes. `/auth` marked `noindex`. `index.html` enriched with base OG/Twitter tags + keywords + canonical. New `/app/frontend/public/sitemap.xml` (11 URLs w/ priority + changefreq), `/app/frontend/public/robots.txt` (allow public routes, disallow /dashboard /habits /focus /coach /admin), `/app/frontend/public/og-cover.png` (Mandala image referenced by all OG tags). Verified per-route titles update, canonical points to production domain, sitemap.xml and robots.txt serve correctly from Vercel build.
- **Feb 10, 2026**: **Polished landscape OG cover** — generated via `/tmp/make_og.py` (PIL script) a proper 1200×630 landscape OG image (`/app/frontend/public/og-cover.png`, 315 KB). Composition: midnight-blue gradient background, left-aligned bold typography ("BUILDING MY AWESOME LIFE DAILY" small caps teal → "Small habits." serif white → "Awesome life." italic teal accent → "Habits · Focus games · Mindfulness · AI Coach" subline → `buildingmyawesomelifedaily.com` gold URL), right-side Mandala with radial-fade mask and 35% saturation boost. Now WhatsApp / Twitter / Facebook / Slack share previews look production-grade.
- **Feb 10, 2026**: **Landing hero text contrast fix** — reduced Mandala watermark opacity from 35% → 18% on landing page, plus added a radial vignette fixed behind the hero copy zone. Extended `.dark .alu-s` scoped dark-mode overrides to include `[data-testid="landing-page"]` so the public landing page renders correctly in both themes.
- **Feb 10, 2026**: **Candle Flame Flicker — full visual redesign** — `/app/frontend/src/components/exercises/CandleFlame.jsx` rewritten to match user-provided reference: cinematic navy background, warm ambient radial glow (blur-24 pulsing), sacred-geometry mandala backdrop (`/app/frontend/src/assets/candle-mandala.png` — generated by feathered-Gaussian-blurring the "25:00" text out of `focus-mandala.png`), all-SVG dripping ivory pillar candle (linear + radial gradients + drip paths + wick), animated multi-layer flame group (outer/mid/core radial gradients) with `candle-flame-flicker` keyframes rotating and scaling to simulate real combustion, 16 memoized spark particles animating upward with individual delays/durations/drift, and `prefers-reduced-motion` support. Verified 100% by testing_agent (iteration_8).
- **Feb 10, 2026**: **Security audit remediation** — Ran `security_audit_agent`; addressed all HIGH + hardening findings:
  - **SEC-001 (HIGH) — Removed 3 unmetered AI endpoints** in `/app/backend/server.py` that could drain the Emergent LLM Key budget: legacy `POST /api/chat`, `POST /api/ai-coach`, `POST /api/glow-up/generate`. All frontend usage already migrated to quota-gated `/api/ai/chat` — no functionality lost.
  - **SEC-002 (MEDIUM) — Pydantic Field validators** on `AIChatRequest`: `message` capped at 1-2000 chars, `system_prompt` at 1200 chars, `temperature` 0-1.5, `max_output_tokens` 32-2048. Blocks arbitrary-prompt/oversized LLM abuse.
  - **CORS hardened** — `/app/backend/.env` `CORS_ORIGINS` changed from `"*"` to `"https://buildingmyawesomelifedaily.com,https://www.buildingmyawesomelifedaily.com,https://mindful-focus-30.preview.emergentagent.com"`. Server-side also defensively filters `'*'` entries so a rogue env can't defeat `allow_credentials=True`. Allowed methods restricted to `GET/POST/PUT/DELETE/OPTIONS`, headers to `Content-Type/Authorization/X-User-Id`.
  - **`/api/health` minimized** — now returns only `{"status":"ok"}` (was leaking key presence, mongo state, and business usage counters).
  - **Generic error messages** on `/api/ai/chat` + `/api/ai/generate-feed` — no more exception echo in HTTP responses.
  - **SEC-004 (LOW) — Firestore rules tightened**: new `validCommunityContent()` (content required, 1-1000 chars string) enforced on `community_posts` create; new `validEngagementIncrement()` enforces `+1` increment on exactly ONE of `likes`/`saves`/`shares` per update (blocks counter spike/tamper) — applied to both `feed_posts` and `community_posts` updates.
  - Verified 100% via testing_agent (iterations 6+7). Preview URL CORS shows `*` due to Cloudflare edge injection but backend itself is correctly locked (localhost curl confirms). Production Render behind Vercel will honor the correct headers.
  - **Deferred** (needs manual user action or future integration): (a) Rotate `EMERGENT_LLM_KEY` + `GEMINI_API_KEY` since they were committed to `.env` (b) Re-publish updated `firestore.rules` in Firebase Console (c) SEC-003 server-side Pro entitlement validation — will be handled by Lemon Squeezy webhook + `/api/subscription/status` endpoint when payments are wired.
